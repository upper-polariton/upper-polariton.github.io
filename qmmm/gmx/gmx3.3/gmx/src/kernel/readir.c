/*
 * $Id: readir.c,v 1.147 2002/09/20 18:16:57 lindahl Exp $
 * 
 ...
 */
static char *SRCID_readir_c = "$Id: readir.c,v 1.147 2002/09/20 18:16:57 lindahl Exp $";
#include <ctype.h>
#include <stdlib.h>
#include "sysstuff.h"
#include "smalloc.h"
...
static char tcgrps[STRLEN],tau_t[STRLEN],ref_t[STRLEN],
  ...
  energy[STRLEN],user1[STRLEN],QMMM[STRLEN],vcm[STRLEN],xtc_grps[STRLEN],
...
static int search_QMstring(char *s,int ng,char *gn[])
{
  ...
  fatal_error(0,"this QM method or basisset (%s) is not implemented\n!",s);
	      ...
	      t_inputrec *ir,t_idef *idef,int *forward,rvec *v)
{
  ...
  int     nacg,nfreeze,nfrdim,nenergy,nQMMM,nuser,negexcl;
  char    *ptr1[MAXPTR],*ptr2[MAXPTR],*ptr3[MAXPTR];
  int     i,j,k,restnm;
  bool    bExcl,bSetTCpar;
  int     
    nQMmethod,nQMbasis,nQMcharge,nQMmult,nbSH,nCASorb,nCASelec,
    nSAon,nSAoff,nSAsteps,nQMg;
  ...
  if((nQMmethod != nQMg)||(nQMbasis != nQMg)){
    fatal_error(0,"Invalid QMMM input: %d groups %d basissets"
