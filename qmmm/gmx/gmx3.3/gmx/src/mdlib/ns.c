/*
 * $Id: ns.c,v 1.61 2002/06/07 07:44:14 hess Exp $
 * 
 ...
 */
static char *SRCID_ns_c = "$Id: ns.c,v 1.61 2002/06/07 07:44:14 hess Exp $";
#ifdef USE_THREADS
...
static void reset_neighbor_list(t_forcerec *fr,bool bLR,int eNL)
{
  ...
  if (bLR) 
    ...
    if(fr->bQMMM)/* only reset the shortrange one */
...
static inline void close_neighbor_list(t_forcerec *fr,bool bLR,int eNL)
{
  ...
  if (bLR) /* will never be true when using QMMM! */
  ...
  */
  ...
  if (fr->efep==efepNO) {
	...
	if (bWater && (fr->solvent_type[jcg] == esolWATER)) {
      ...
      /* MNO solvent as i charge group, no free energy AND NO QM! */

      /* IN case the QM subsystem contains only a single charge group,
       * it would be selected for optimizad solvent (MNO). In
       * check_solvent we therefore check if the particle is a QM
       * partilce (bQMMM).
       */
      i_atom  = i0;
      igid    = cENER[i_atom];
    ...
    } else if (bQMMM) { /* QMMM atom as i charge group */
      /* Loop over the atoms in the i charge group */    
      for(i=0; i<nicg; i++) {
	i_atom  = i0+i;
	igid    = cENER[i_atom];
	...
	for(j=0; (j<nj); j++) {
	  ...
	  /* Charge groups can not have QM and MM atoms simultaneously */
	  if (jcg != icg) {
	    jj0 = index[jcg];
	    jj1 = index[jcg+1];
	    /* Finally loop over the atoms in the j-charge group */	
		       ...
		       int shift,t_forcerec *fr)
{
  if (nsbuf->ncg >= MAX_CG) {
    put_in_list(bHaveLJ,ngid,md,icg,jgid,nsbuf->ncg,nsbuf->jcg,
		cgs->index,/* cgs->a, */ bexcl,shift,fr,
...
static int ns_simple_core(t_forcerec *fr,
			  ...
			  bool bHaveLJ[])
{
  /* ! needs to be adapted for QMMM stuff */
...
static void do_longrange(FILE *log,t_commrec *cr,t_topology *top,t_forcerec *fr,
			 ...
			 bool bDoForces,bool bHaveLJ[],bool bQMMM)
{
  ...
  for(i=0; (i<eNL_NR); i++) {
    if ((fr->nlist_lr[i].nri > fr->nlist_lr[i].maxnri-32) || bDoForces) {
      ...
      /* Evaluate the forces */
      if (bQMMM)
	fprintf(stderr,"there is QMMM, but still do_fnbf is called, ai ai ai\n");
  ...
  if (!bDoForces) {  
    ...
    put_in_list(bHaveLJ,ngid,md,icg,jgid,nlr,lr,top->blocks[ebCGS].index,
		...
		TRUE,bVDWOnly,bCoulOnly,bQMMM); /* FAlse, since no QMMM */
  }
}
...
static int ns5_core(FILE *log,t_commrec *cr,t_forcerec *fr,int cg_index[],
		    ...
		    real lambda,real *dvdlambda,
		    bool bHaveLJ[], bool bMakeQMMMnblist)
{
  ...
  for(iicg=fr->cg0; (iicg < fr->hcg); iicg++) {
    ...
    if(bMakeQMMMnblist){ 
      ...
      if (fr->qr->bQMMM[cgs->a[cgs->index[icg]]]==FALSE)
	...
	continue;
      ...
      icg_naaj = icg+naaj;
      min_icg = cgsnr;       
    } else { /* make a normal neighbourlist */
	  ...
	  for(nn=0; (nn<ngid); nn++) {
	    ...
	    if (nlr_one[nn] > 0) 
	      do_longrange(log,cr,top,fr,ngid,md,icg,nn,nlr_one[nn],
			   ...
			   lambda,dvdlambda,grps,
			   bVDWOnly,bCoulOnly,FALSE,bHaveLJ,bMakeQMMMnblist);
	  }
	}
      }
    }
  ...
  for (nn=0; (nn<ngid); nn++) {
    ...
    if (rl2 > rm2)
      do_longrange(log,cr,top,fr,0,md,icg,nn,nlr_one[nn],
		   nl_lr_one[nn],bexcl,shift,x,box_size,nrnb,
		   lambda,dvdlambda,grps,bVDWOnly,bCoulOnly,TRUE,
		   bHaveLJ,bMakeQMMMnblist);
  }
  debug_gmx();
		      ...
		      real lambda,real *dvdlambda)
{
  ...
  if (bGrid){
    nsearch = ns5_core(log,cr,fr,cg_index,box,box_size,ngid,top,grps,
		       grid,x,bexcl,bExcludeAlleg,nrnb,md,lambda,
		       dvdlambda,bHaveLJ,FALSE);
