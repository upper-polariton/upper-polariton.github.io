/*
 * $Id: constr.c,v 1.35 2002/08/01 21:56:52 lindahl Exp $
 * 
 ...
 */
static char *SRCID_constr_c = "$Id: constr.c,v 1.35 2002/08/01 21:56:52 lindahl Exp $";
#ifdef HAVE_CONFIG_H
#include <config.h>
#endif
			    ...
			    bool bInit,t_nrnb *nrnb,bool bDumpOnError)
{
  ...
  if (bInit) {
	...
	cconerr(&p_max,&p_rms,&p_imax,xprime,nc,bla1,bla2,bllen);
      ...
      if ((ir->eI == eiSteep) || (ir->eI == eiCG) || (ir->eI == eiTS) || (ir->eI == eiOPT) || bItEqOrder)
