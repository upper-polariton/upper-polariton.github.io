/*
 * $Id: force.c,v 1.72 2002/08/01 21:56:52 lindahl Exp $
 * 
			  ...
			  t_mdatoms *md,t_nsborder *nsb)
{
   ...
   * solvent in the innerloops. The routine should be called once at
   * the beginning of the MD program. NOTE: QM particle should not
  ...
  for(i=0; (i<mols->nr); i++) {
	...
	/* Last check for perturbed atoms and/or QM atoms*/
	if(fr->bQMMM){
	  for(j=j0; (j<j1); j++){
	    if (md->bPerturbed[mols->a[j]]|| fr->qr->bQMMM[mols->a[j]]){
	      /* QM groups that happen to be in a single charge group
	       * must never become an optimized solvent. This saves us a
	       * couple of checks in the ns, and therefore speed up the
	       * ns routines.  
	       */
...
#define TAB_EXT 1.2
  ...
  fr->bQMMM      = ir->bQMMM;    /* always initialize!!!!! */
  fr->qr         = mk_QMMMrec();
