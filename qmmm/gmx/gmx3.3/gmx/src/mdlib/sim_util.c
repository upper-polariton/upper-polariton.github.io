/*
 * $Id: sim_util.c,v 1.53 2002/09/23 10:57:57 spoel Exp $
 * 
 ...
 */
static char *SRCID_sim_util_c = "$Id: sim_util.c,v 1.53 2002/09/23 10:57:57 spoel Exp $";
#include <stdio.h>
#include <time.h>
#include "typedefs.h"
#include "string2.h"
	      ...
	      bool bGatherOnly,real t)
{
  ...
  if (fr->ePBC != epbcNONE) { 
    ...
    } else if (parm->ir.eI==eiSteep || parm->ir.eI==eiCG || parm->ir.eI==eiTS || (parm->ir.eI == eiOPT) )
  ...
  /* also update the QMMM rec if necessary */
  if(fr->bQMMM)
    update_QMMMrec(cr,fr,x,mdatoms,parm->box,top);
